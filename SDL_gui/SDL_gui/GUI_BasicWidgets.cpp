...
void GUI_ListCell::draw()
{
    ...
    if( !display_cmd ) {
        ...
        if( selected ) {
            ...
            {
                SDL_SetTextureColorMod( titleTexture, 192, 192, 192 );
                GUI_FillRect2( GUI_MakeRect(0, 0, tw_area.w, tw_area.h), cLightGrey );
            }
        }
        else {
            if(enable){
...
void GUI_List::setEnable(int cell, bool enable){
...
Uint32 GUI_List_TimerCallback( Uint32 interval, void *param)
{
    ...
    SDL_TimerID tid = l->timerID;
    std::cout << "TimerCallback tid: " << tid << std::endl;
    l->timerID = -1;
    l->disable = false;
    //GUI_Log( "Enable\n" );
    ...
    std::cout << "PushEvent GUI_LISTSELECTED: " << l << std::endl;
...
void GUI_List::selectChanged( int n )
{
    ...
    p = n;
    disable = true;
    std::cout << "ListSelected: " << n << "timeId: " << timerID << std::endl;
    if( timerID != -1 ) {
        std::cout << "RemoveTimer: " << timerID << std::endl;
        SDL_RemoveTimer( timerID );
    }
    ...
    */
    ...
    std::cout << "AddTimer: " << timerID << std::endl;
#endif
}
...
//        GUI_Log( "Reject\n" );
    ...
    switch (ev->type) {
        ...
        case GUI_LISTSELECTED: {
            std::cout << "HandleEvent GUI_LISTSELECTED: " << ev->user.data1 << " by: " << this << std::endl;

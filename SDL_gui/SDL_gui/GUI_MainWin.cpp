...
#ifdef _WIN32
// pom
#define _CRT_SECURE_NO_WARNINGS // ignore error C4996
#endif
...
//    dir.listDir(jsFilePath::getUserHomeDir().c_str());
    dir.listDir(GUI_getDocumentDirectory());
}
...
//			if (file->isDirectory()) {
//				dirList[str_count++] = _strdup(file->getFileName().c_str());
//			}
//			else if (file->isFile()) {
...
void GUI_FileDialog::draw()
{
	if (bNeedUpdate) {
		...
		bttnOK = new GUI_Button(this, tt, tw_area.w - 52, 4, 48, 30, cGrey,
			...
			GUI_FileDialog *dlg = (GUI_FileDialog *)b->parent->parent;
			if (dlg->fileEdit->title_str && dlg->fileEdit->title_str[0]) {
				string path = dlg->dir.path() + dlg->fileEdit->title_str;
				GUI_Log("File: %s\n", path.c_str());
				if (dlg->file_cmd) {
		...
		int dir_count = 0;
		int file_count = 0;
        bool dirDenied[dir.size()];
        bool fileDenied[dir.size()];
		for (auto file = dir.begin(); file != dir.end(); file++) {
			if (file->isDirectory() && dir_count<max_dir-1) {
                if(file->canRead()){
			...
			else if (file->isFile() && file_count<max_dir-1) {
                if(file->canRead()){
		...
		dirListBox = new GUI_List(this, (const char **)dirList, dir_count, -1, by - 1, 180 + 2, this->tw_area.h - titleBarSize + 1 - by, 0, false,
			[](GUI_List *l, const char *sel, int index) {
...
//                auto perm = std::filesystem::status(jsFile(path)).permissions();
		...
		dirListBox->title_str = "Dir List Box";
        for(int i=0; i<dirListBox->numCells; i++){
		...
		fileListBox->title_str = "File List Box";
        for(int i=0; i<fileListBox->numCells; i++){
		...
		char cwd[512];
        strcpy(cwd, dir.path().c_str());
//        strcpy(cwd, jsFilePath::getCurrentWorkingDirectory().c_str());
        
//        strcpy(cwd, jsFilePath::addLeadingSlash(dir.path()).c_str());
//        if(cwd[0]!='/' && cwd[0]!='\\'){
//            #ifdef _WIN32
//                sprintf(cwd, "\\%s", cwd);
//            #else
...
#endif
			{
//				if (!bFirstTrailingSlash) {
					dirName[fc++] = &cwd[i + 1];
//				}
				cwd[i] = 0;
//				bFirstTrailingSlash = false;
				if (fc >= max_dir - 1)
					break;
			}
		}
		...
		dirButton = new GUI_Button(titleBar, dirName[1], 10, 0, 180, 0, cClear,
			[](GUI_Button *bt) {
			GUI_FileDialog *dlg = (GUI_FileDialog *)bt->parent->parent;
			if (dlg->menuDir->isVisible())
				dlg->menuDir->hide();
		...
		menuDir = new GUI_List(this, (const char **)&dirName[2], fc,
			10, 1, 160, 0, 0, true,
			[](GUI_List *l, const char *sel, int index) {
				//GUI_Log( "Selected: %s %i\n", sel, index);
				...
				GUI_FileDialog *dlg = (GUI_FileDialog *)l->parent->parent;
				char path[512];
                strcpy(path, dlg->dir.path().c_str());
//                strcpy(path, dlg->dir.getAbsolutePath().c_str());

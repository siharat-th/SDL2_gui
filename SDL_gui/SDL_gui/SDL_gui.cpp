...
#else
	std::string fontPath = getResourcePath("") + GUI_FONTPATH;
	std::string fontPathMono = getResourcePath("") + GUI_FONTPATH_MONO;

	if (!(GUI_title_font = TTF_OpenFont(fontPath.c_str(), GUI_nominal_font_size + 2)))  // fontpath from config.h
	{
		GUI_Log("font-spec %s not found", fontPath.c_str());
		return NULL;
	}
	if (!(GUI_def_font = TTF_OpenFont(fontPath.c_str(), GUI_nominal_font_size)))
	{
		GUI_Log("font-spec %s not found", fontPath.c_str());
		return NULL;
	}
	if (!(GUI_small_font = TTF_OpenFont(fontPath.c_str(), GUI_nominal_font_size - 3)))
	{
		GUI_Log("font-spec %s not found", fontPath.c_str());
		return NULL;
	}
	if (!(GUI_mono_font = TTF_OpenFont(fontPathMono.c_str(), GUI_nominal_font_size)))
	{
		GUI_Log("font-spec %s not found", fontPathMono.c_str());
		return NULL;
	}
#endif
...
//#ifndef __sdl_color__
//#define __sdl_color__

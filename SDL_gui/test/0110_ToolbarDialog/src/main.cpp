...
Uint32 OpenFileCallback( Uint32 interval, void *param)
{
    new GUI_FileDialog( topWin, false, "*", "Untitled.dat" );
    return 0;
}

void OpenFile( void ) {
    new GUI_FileDialog( topWin, false, "*", "Untitled.dat", [](const char *fn) -> bool {
...
void savePalette( const char *fn ) {
	...
	for (int i = 0; i<numPalette; i++) {
		for (int j = 0; j<numColor; j++) {
			Uint32 c = (mainPalette[i][j].r << 24) | (mainPalette[i][j].g << 16) | (mainPalette[i][j].b << 8) | 0xFF;
			buff.append((char*)&c, sizeof(Uint32));
		}
	}
	jsBufferToFile(fn, buff, true);
}
...
void loadPalette( const char *fn ) {
	...
	jsBuffer buffer = jsBufferFromFile(fn, true);
	if (buffer.size()) {
		Uint32* data = (Uint32*)buffer.getData();
		for (int i = 0; i<numPalette; i++) {
			for( int j=0; j<numColor; j++ ) {
				Uint32 c = *data;
				mainPalette[i][j] = sdl_color(c);
...
{
    new GUI_FileDialog( topWin, true, "pal", "Untitled.pal", [](const char *fn) -> bool {
...
{
    new GUI_FileDialog( topWin, false, "pal", "Untitled.pal",  [](const char *fn) -> bool {
        GUI_Log( "File Load Pal: %s\n", fn );
        loadPalette( fn );
        for( int i=0; i<numColor; i++ ) {
                    ...
                    menuFile = new GUI_List( topWin, menuText, 4, bt->tw_area.x, bt->tw_area.y+bt->tw_area.w+10, 240, 0, 0, true,
                        [](GUI_List *l,const char *sel, int index) {
                            switch (index) {
                                case 1:
                                    OpenFile();
//                                    SDL_AddTimer( 100, OpenFileCallback, NULL);
                                    break;
                                case 3:
